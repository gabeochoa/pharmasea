...
static void ShutdownSteamDatagramConnectionSockets() {
    // Give connections time to finish up.  This is an application layer
    // protocol here, it's not TCP.  Note that if you have an application and
    // you need to be more sure about cleanup, you won't be able to do this. You
    // will need to send a message and then either wait for the peer to close
    // the connection, or you can pool the connection to see if any reliable
    // data is pending.
   ...
   public:
    void Run(uint16 nPort) {
        ...
        for (auto it : m_mapClients) {
            ...
            // that's usually best left for more diagnostic/debug text not
            // actual protocol strings.
   ...
   private:
    ...
    void PollIncomingMessages() {
        ...
        while (!g_bQuit) {
            ...
            int numMsgs = m_pInterface->ReceiveMessagesOnPollGroup(
                m_hPollGroup, &pIncomingMsg, 1);
            if (numMsgs == 0) break;
            ...
            std::string sCmd;
            sCmd.assign((const char *) pIncomingMsg->m_pData,
            ...
            if (strncmp(cmd, "/nick", 5) == 0) {
                ...
                sprintf(temp, "%s shall henceforth be known as %s",
                        itClient->second.m_sNick.c_str(), nick);
            ...
            // Assume it's just a ordinary chat message, dispatch to everybody
            // else
    ...
    void OnSteamNetConnectionStatusChanged(
        ...
        switch (pInfo->m_info.m_eState) {
            ...
            case k_ESteamNetworkingConnectionState_Connecting: {
                ...
                if (m_pInterface->AcceptConnection(pInfo->m_hConn) !=
                    k_EResultOK) {
                ...
                if (m_mapClients.empty()) {
                    ...
                    sprintf(temp, "%d companions greet you:",
        ...
        switch (pInfo->m_info.m_eState) {
            ...
            case k_ESteamNetworkingConnectionState_ProblemDetectedLocally: {
                ...
                if (pInfo->m_eOldState ==
                    ...
                    Printf(
                        "We sought the remote host, yet our efforts were met "
                        "with defeat.  (%s)",
                        pInfo->m_info.m_szEndDebug);
                } else if (
...
int main(int argc, const char *argv[]) {
    ...
    // Ug, why is there no simple solution for portable, non-blocking console
    // user input? Just nuke the process
    // LocalUserInput_Kill();
    NukeProcess(0);
}
