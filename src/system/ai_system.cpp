...
[[nodiscard]] bool entity_ref_valid(const EntityRef& ref) {
...
[[nodiscard]] OptEntity find_best_toilet_with_space(const Entity& ai_entity) {
    return EntityQuery()
...
[[nodiscard]] OptEntity find_best_jukebox_with_space(const Entity& ai_entity) {
    return EntityQuery()
...
void set_state(Entity& e, IsAIControlled::State next) {
    e.get<IsAIControlled>().state = next;
}

void wander_pause(Entity& e, IsAIControlled::State resume) {
    IsAIControlled& ctrl = e.get<IsAIControlled>();
    ctrl.resume_state = resume;
    ctrl.state = IsAIControlled::State::Wander;
...
void enter_bathroom(Entity& entity, IsAIControlled::State return_to) {
    set_state(entity, IsAIControlled::State::Bathroom);
...
void process_ai_entity(Entity& entity, float dt) {
    ...
    switch (ctrl.state) {
        case IsAIControlled::State::Wander: {
            ...
            tgt.pos.reset();
            set_state(entity, ctrl.resume_state);
        } break;
        ...
        case IsAIControlled::State::QueueForRegister: {
            ...
            set_state(entity, IsAIControlled::State::AtRegisterWaitForDrink);
        } break;
        ...
        case IsAIControlled::State::AtRegisterWaitForDrink: {
            ...
            if (!opt_reg) {
                tgt.entity.clear();
                set_state(entity, IsAIControlled::State::QueueForRegister);
                return;
            }
            Entity& reg = opt_reg.asE();
            ...
            set_state(entity, IsAIControlled::State::Drinking);
        } break;
        ...
        case IsAIControlled::State::Drinking: {
            ...
            if (!want_another) {
                ...
                set_state(entity, IsAIControlled::State::Pay);
                return;
            }
        ...
        case IsAIControlled::State::Pay: {
            ...
            set_state(entity, IsAIControlled::State::Leave);
        } break;
        ...
        case IsAIControlled::State::Bathroom: {
            if (entity.is_missing<CanOrderDrink>()) {
                set_state(entity, IsAIControlled::State::Wander);
                return;
            }

            if (!needs_bathroom_now(entity)) {
                HasAIBathroomState& bs = ensure_component<HasAIBathroomState>(entity);
                set_state(entity, bs.next_state);
