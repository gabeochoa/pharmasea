...
void SystemManager::process_state_change(
    ...
    for (const auto& transition : transitions) {
        ...
        if (old_state == game::State::Lobby) {
            // TODO - NOTE that anything generated by the store is considered
            // real for "in round" since we dont generally filter out store
            // items from EntitiyQueries
            // - This caused issues where registers were being spawned in the
            // store and customers would try to path to it (this was fixed by
            // changing pathing)
            // - We probably want to add a <ForSale> component that will
            // automatically be filtered out for most queries or something
            // system_manager::store::generate_store_options();
        }
