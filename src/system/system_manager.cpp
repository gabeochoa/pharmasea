...
void update_held_furniture_position(Entity& entity, float) {
    ...
    asE(can_hold_furniture.furniture()).get<Transform>().update(new_pos);
}
...
void highlight_facing_furniture(Entity& entity, float) {
    ...
    // TODO better names
    asE(match).get<CanBeHighlighted>().update(true);
}
...
void process_conveyer_items(Entity& entity, float dt) {
    ...
    auto match = EntityHelper::getClosestMatchingFurniture(
        transform, 1.f, [&entity](Entity furn) {
            // cant be us
...
void backfill_empty_container(Entity& entity, TArgs&&... args) {
...
void sophie(Entity& entity, float) {
    ...
    // Handle customers finally leaving the store
    {
        ...
        for (OptEntity e : customers) {
            if (!valid(e)) continue;
            if (vec::distance(asE(e).get<Transform>().as2(), endpos) >
                TILESIZE * 2.f) {
                all_gone = false;
                break;
            }
