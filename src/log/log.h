...
// Overload matching afterhours signature (Duration, log_level, fmt_str,
// args...) Used by vendor/afterhours/src/plugins/window_manager.h
...
inline void log_once_per(Duration /*duration*/, int log_level,
    ...
    if (log_level == 3)
        level = LogLevel::LOG_WARN;  // VENDOR_LOG_WARN
    else if (log_level == 4)
        level = LogLevel::LOG_ERROR;  // VENDOR_LOG_ERROR
