...
raylib::Font Preload::load_karmina_regular() {
    auto ffont = font::LoadFont_KarminaRegular256();
    raylib::GenTextureMipmaps(&ffont.texture);
    raylib::SetTextureFilter(ffont.texture, raylib::TEXTURE_FILTER_POINT);
    return ffont;
}
...
void Preload::load_music() {
    MusicLibrary::get().load(
        Files::get()
            .fetch_resource_path(strings::settings::MUSIC, "supermarket.ogg")
            .c_str(),
        "supermarket");
