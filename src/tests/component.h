...
inline Entity& mk_entity() {
    return make_entity(EntityHelper::createEntity(),
                       DebugOptions{.name = "test"}, vec3{0, 0, 0});
}
...
void validate_wall_color_serialized() {
    Entity& entity = entities::make_wall(EntityHelper::createEntity(),
                                         vec2{0, 0}, ui::color::saffron);

    M_TEST_T(entity.has<SimpleColoredBoxRenderer>(), "should have renderer");

    M_TEST_EQ(entity.get<SimpleColoredBoxRenderer>().face(), ui::color::saffron,
              "component should have right color");

    M_TEST_EQ(entity.get<SimpleColoredBoxRenderer>().base(), ui::color::saffron,
              "component should have right color");
...
void validate_trigger_area_serialized() {
    Entity& entity = entities::make_trigger_area(EntityHelper::createEntity(),
                                                 vec3{0, 0, 0}, 10, 20, "test");
