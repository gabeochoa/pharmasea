...
struct BasePlayer : public Person {
    ...
    BasePlayer(raylib::vec3 p, raylib::Color face_color_in,
               raylib::Color base_color_in)
        : Person(p, face_color_in, base_color_in) {}
    BasePlayer(raylib::vec2 p, raylib::Color face_color_in,
               raylib::Color base_color_in)
        : Person(p, face_color_in, base_color_in) {}
    BasePlayer() : Person({0, 0, 0}, {0, 255, 0, 255}, {255, 0, 0, 255}) {}
    BasePlayer(raylib::vec2 location)
        : Person({location.x, 0, location.y}, {0, 255, 0, 255},
                 {255, 0, 0, 255}) {}

    virtual float base_speed() override { return 7.5f; }

    virtual raylib::vec3 update_xaxis_position(float dt) override = 0;
    virtual raylib::vec3 update_zaxis_position(float dt) override = 0;
