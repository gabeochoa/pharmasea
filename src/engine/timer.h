    ...
    class impl {
        ...
        void timer() {
            ...
            while (enabled && max_iter != 0) {
                if (cvar.wait_until(lock, deadline) ==
...
interval_timer<typename std::decay<Function>::type> make_interval_timer(
