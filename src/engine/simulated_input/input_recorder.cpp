...
void record(const Event& event) {
    ...
    switch (type) {
        case EventType::KeyPressed: {
            ...
            const int keycode = key_event.getKeyCode();
            if (keycode >= 0 &&
                keycode < static_cast<int>(state.key_down.size())) {
                if (!state.key_down[keycode]) {
                    state.key_down[keycode] = true;
                    write_line(timestamp, "key", std::to_string(keycode),
                               "down");
                    log_info("recorder: key {} down", keycode);
                }
            } else {
                write_line(timestamp, "key", std::to_string(keycode), "down");
                log_info("recorder: key {} down (no track)", keycode);
            }
            break;
        }
        case EventType::KeyReleased: {
            ...
            if (keycode >= 0 &&
                ...
                if (state.key_down[keycode]) {
                    ...
                    write_line(timestamp, "key", std::to_string(keycode), "up");
                    log_info("recorder: key {} up", keycode);
                }
            } else {
                write_line(timestamp, "key", std::to_string(keycode), "up");
                log_info("recorder: key {} up (no track)", keycode);
            }
            break;
        }
        case EventType::MouseButtonDown:
        case EventType::MouseButtonPressed: {
            const Mouse::MouseButtonEvent& mouse_event =
                static_cast<const Mouse::MouseButtonEvent&>(event);
            const int button_code =
                static_cast<int>(mouse_event.GetMouseButton());
            if (button_code >= 0 &&
        ...
        case EventType::MouseMoved: {
            const Mouse::MouseMovedEvent& mouse_event =
                static_cast<const Mouse::MouseMovedEvent&>(event);
            float x = mouse_event.x();
            float y = mouse_event.y();
            if (x != state.last_mouse_x || y != state.last_mouse_y) {
